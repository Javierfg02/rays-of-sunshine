...
void Realtime::initializeGL() {
    ...
    // buoilding data
    std::vector<float> allBuildingData;
    const int numBuildings = 20;

    for (int i = 0; i < numBuildings; i++) {
        // random building parameters
        int floors = 3 + rand() % 8;  // 3 to 10 floors
        int windows = 2 + rand() % 3;  // 2 to 4 windows
        float height = 0.8f + (rand() % 100) / 100.0f * 0.8f;
        float width = 1.0f + (rand() % 100) / 100.0f;
        float depth = 1.0f + (rand() % 100) / 100.0f;

        Building* building = new Building();
        building->updateParams(floors, windows, height, width, depth);
        std::vector<float> buildingData = building->generateShape();

        // position buildings in a curve
